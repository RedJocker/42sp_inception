...
- [ ] Wordpress database must have two users with one administrator
- [ ] Nginx must be the only entrypoint to services via 443 using TLSv1.[23]
- Forbiden
  - [ ] using latest tag on containers
  - [ ] exponsing sensitive data(passwords, credenntials, keys)
  - [ ] not using .env
...
- [ ] extract variables
  - [ ] extract variables to .env
    ...
    - [ ] use envsubst to replace varibles from other .conf files
